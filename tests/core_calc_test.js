...
test('配股（股票股利）：零成本股攤薄均價、賣出時實現為獲利', function(){
  trades=[
    ['2024/01/02','2330','買進',1000,600,855,0,0],   // 成本 600,000
    ['2024/06/20','2330','配股',100,0,0,0,0],          // 配 100 股，零成本
  ];
  computeAll();
  eq(holdings['2330'].qty, 1100, '持股 = 1000 + 配股 100');
  // 總成本不變 600,000 ÷ 1100 股 ⇒ 均價攤薄
  near(holdings['2330'].avgCost, 600000/1100, 1e-6, '均價被攤薄');
  eq(realizedList.length, 0, '配股本身不產生已實現損益');

  // 賣掉全部：FIFO 先吃 1000@600，再吃 100@0（零成本）⇒ 零成本那批全為獲利
  trades.push(['2024/07/01','2330','賣出',1100,700,0,0,0]);
  computeAll();
  eq(realizedList.length, 1, '一筆已實現');
  near(realizedList[0].gross, 1100*700 - (1000*600 + 100*0), 1e-6, '毛損益＝賣出收入−買進本金(含零成本股)');
  eq(Object.keys(holdings).length, 0, '全部賣光');
});
